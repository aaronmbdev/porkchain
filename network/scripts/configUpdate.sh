...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer.meatchain.cloud:7050 --ordererTLSHostnameOverride orderer.meatchain.cloud -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
